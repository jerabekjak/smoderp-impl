...
#import smoderp2d.src.data_preparation
import smoderp2d.src.flow_algorithm.arcgis_dmtfce as arcgis_dmtfce
...
def prepare_streams(dmt, dmt_copy, mat_dmt_fill, null,
    ...
    hranice2 = arcpy.Buffer_analysis(
        hranice, temp_dp+os.sep+"hranice2.shp", -spix/3, "FULL", "ROUND", "NONE")
    #hranice2 = hranice
    ...
    arcpy.DeleteField_management(toky, ["SHAPE_LEN", "SHAPE_LENG", "SHAPE_LE_1", "NAZ_TOK_1",
                                        "TOK_ID_1", "SHAPE_LE_2", "SHAPE_LE_3", "NAZ_TOK_12", "TOK_ID_12", "SHAPE_LE_4", "ORIG_FID_1"])
    ...
    arcpy.SelectLayerByAttribute_management(
        toky_t, "NEW_SELECTION", "RASTERVALU < RASTERVA_1")
    arcpy.FlipLine_edit(toky_t)
    # arcpy.DeleteField_management(toky_t, ["RASTERVALU","RASTERVA_1","ORIG_FID","ORIG_FID_1"])
    arcpy.DeleteField_management(
    ...
    start_point_check = arcpy.sa.ExtractValuesToPoints(
        start, dmt, temp_dp+os.sep+"start_point_check", "NONE", "VALUE_ONLY")
    ...
    arcpy.DeleteField_management(toky, ["SHAPE_LEN", "SHAPE_LE_1", "SHAPE_LE_2", "SHAPE_LE_3", "SHAPE_LE_4", "SHAPE_LE_5", "SHAPE_LE_6",
                                        "SHAPE_LE_7", "SHAPE_LE_8", "SHAPE_LE_9", "SHAPE_L_10", "SHAPE_L_11", "SHAPE_L_12", "SHAPE_L_13", "SHAPE_L_14"])
    arcpy.DeleteField_management(
        toky, ["ORIG_FID", "ORIG_FID_1", "SHAPE_L_14"])

    stream_rst1 = temp_dp+os.sep+"stream_rst"
    stream_rst = arcpy.PolylineToRaster_conversion(
        toky, "FID", stream_rst1, "MAXIMUM_LENGTH", "NONE", spix)
    tok_usek = temp_dp+os.sep+"tok_usek"

    # jj no data 1000 kdyz in useku bude od 10000
    arcpy.gp.Reclassify_sa(stream_rst, "VALUE",
                           "NoDataValue 1000", tok_usek, "DATA")
    mat_tok_usek = arcpy.RasterToNumPyArray(
        temp_dp+os.sep+"tok_usek", ll_corner, cols, rows)
    ...
    # sklon
    fc = toky
    field = ["FID", "RASTERVALU", "RASTERVA_1",
    ...
    try:
        arcpy.JoinField_management(toky, tab_stream_tvar_code, stream_tvar_dbf,
                                   tab_stream_tvar_code, "cislo;tvar;b;m;drsnost;Q365")
    except:
