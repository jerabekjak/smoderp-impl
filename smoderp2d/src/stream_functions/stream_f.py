...
def compute_h(A, m, b, err=0.0001, maxIter=20):
    ...
    while (feval(h_pre) > err):
        ...
        h_pre = h
        if iter_ >= maxIter:
            prt.error("if file", frameinfo.filename, "near line ", frameinfo.lineno,
                      "\n\t newton solver didnt converge after", maxIter, 'iterations (maxIter=', maxIter, ')')
            break
        iter_ += 1
...
def rectangle(reach, dt):
    ...
    reach.vs = math.pow(R, 0.6666)*math.pow(reach.slope,
                                            0.5)/(reach.roughness)  # rychlost
...
def trapezoid(reach, dt):
    ...
    Bb = B+hp*reach.m
    h = compute_h(A=(reach.V_in_from_field + reach.V_rest +
                     reach.V_in_from_reach)/reach.length, m=reach.m, b=reach.b)
    ...
    if reach.V_out > dV:
        ...
        reach.V_rest = dV - reach.V_out  # V_zbyt
    reach.h = H
    #prt.mujout.writelines(str(reach.id_) + ';' + str(reach.h) + ';' + str(reach.V_in_from_field) + ';' + str(reach.V_rest) + ';' + str( reach.V_in_from_reach) + ';' + str(reach.V_out) + ';' + str(reach.to_node)+'\n')
...
def triangle(reach, dt):
    ...
    Ve = (reach.V_in_from_field + reach.V_rest +
