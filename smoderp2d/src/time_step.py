        ...
        for i in rr:
            for j in rc[i]:
                ...
                if surface_state >= 1000:
                    ...
                    q_sheet, v_sheet, q_rill, v_rill, ratio, rill_courant = runoff(
                ...
                co = 'sheet'
                courant.CFL(i, j, surface.arr[i][j].h_total_pre, v,
                            delta_t, mat_efect_vrst[i][j], co, rill_courant)
                rill_courant = 0.
    ...
    def do_next_h(self, rr, rc, pixel_area, surface, subsurface, rain_arr, cumulative, hydrographs, rainfall, courant, total_time, delta_t, combinatIndex, NoDataValue, sum_interception, mat_efect_vrst, ratio, iter_):
        ...
        for i in rr:
            for j in rc[i]:
                ...
                NS, sum_interception, rain_arr.arr[i][j].veg_true = rain_f.current_rain(
                    rain_arr.arr[i][j], rainfall, sum_interception)
