...
def prepare_data(args):
  ...
  dmt_copy = functions.zapis("dmtcopy1",copydmt_Array,ll_corner,spix,vpix,NoDataValue,temp)"""
    ...
    def addfield(inpute, newfield, datatyp, default_value):  # EDL
        ...
        arcpy.CalculateField_management(
            inpute, newfield, default_value, "PYTHON")
        return inpute
    ...
    arcpy.JoinField_management(intersect, "puda_veg", puda_veg_dbf,
                               tab_puda_veg_code, "k;s;n;pi;ppl;ret;b;x;y;tau;v")
  ...
  """
    ...
    zapis("fl_dir", mat_fd, x_coordinate,
          y_coordinate, spix, vpix, NoDataValue, temp)
    ...
    all_attrib = [mat_k, mat_s, mat_n, mat_ppl, mat_pi, mat_ret, mat_b,
                  mat_x, mat_y, mat_tau, mat_v]  # parametry, ktere se generuji ze shp
    poradi = 0
    ...
    #dmt_fill,flow_direction,flow_accumulation,slope = functions.dmtfce(dmt_clip, temp,"TRUE", "TRUE", "NONE")
    ...
    if points and points != "#":
        for kyk in range(array_points.shape[0] - 1):
            if array_points[kyk][1] == i and array_points[kyk][2] == j:
                gp.AddMessage("Point FID = " + str(int(
                    array_points[kyk][0])) + " is at the edge of the raster. This point will not be included in results.")
  ...
  rmat_hcrit.save(output+os.sep+"hcrit")"""
    ...
    if (type_of_computing == 3) or (type_of_computing == 5):
        ...
        import smoderp2d.src.stream_functions.stream_preparation as sp
