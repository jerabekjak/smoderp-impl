...
def dmtfce(dmt, save_dir, fill, filtr, fl_dir):
    ...
    try:
        ...
        arcpy.AddMessage(
            "Unexpected error during raster fill calculation:", sys.exc_info()[0])
        raise
    ...
    try:  # tady by pak meslo jit pridavat pripraven fl dir
        ...
        arcpy.AddMessage(
            "Unexpected error during flow direction calculation:", sys.exc_info()[0])
        raise
    ...
    try:
        ...
        arcpy.AddMessage(
            "Unexpected error during flow accumulation calculation:", sys.exc_info()[0])
        raise
    ...
    try:
        ...
        arcpy.AddMessage(
            "Unexpected error during slope calculation:", sys.exc_info()[0])
        raise
