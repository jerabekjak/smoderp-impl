# @package smoderp2d.main_classes.Surface
#
...
class Surface(Stream if stream == True else StreamPass, Kinematic, Globals, Size):
    ...
    def return_str_vals(self, i, j, sep, dt, extraOut):
        ...
        if not(extraOut):
            line = str(arr.h_total_new) + sep + str(arr.V_runoff/dt +
                                                    arr.V_runoff_rill/dt) + sep + str(arr.V_runoff+arr.V_runoff_rill)
            bil_ = ''
        else:
            line = str(arr.h_sheet) + sep + str(arr.V_runoff/dt) + sep + str(arr.V_runoff) + sep + str(arr.V_rest) + sep + str(
                arr.infiltration) + sep + str(arr.cur_sur_ret) + sep + str(arr.state) + sep + str(arr.inflow_tm) + sep + str(arr.h_total_new)

            if self.rill_computing:

                line += sep + str(arr.h_rill) + sep + str(arr.rillWidth) + sep + str(arr.V_runoff_rill/dt) + sep + str(arr.V_runoff_rill) + sep + str(
                    arr.V_rill_rest) + sep + str(arr.V_runoff/dt + arr.V_runoff_rill/dt) + sep + str(arr.V_runoff+arr.V_runoff_rill) + sep

            bil_ = arr.h_total_pre*self.pixel_area + arr.cur_rain*self.pixel_area + arr.inflow_tm - (arr.V_runoff + arr.V_runoff_rill + arr.infiltration*self.pixel_area) - (
                arr.cur_sur_ret*self.pixel_area) - arr.h_total_new*self.pixel_area  # << + arr.V_rest + arr.V_rill_rest) + (arr.V_rest_pre + arr.V_rill_rest_pre)
...
def rill_runoff(i, j, sur, dt, efect_vrst, ratio):
    ...
    h, b = rill.update_hb(V_to_rill, constants.RILL_RATIO,
                          efect_vrst, sur.rillWidth, ratio, ppp)
    R_rill = (h*b)/(b + 2*h)
    #print '\t', h,b, b, 2*h
    v_rill = math.pow(R_rill, (2.0/3.0)) * 1. / \
        Gl.mat_n[i][j] * math.pow(Gl.mat_slope[i][j]/100, 0.5)
