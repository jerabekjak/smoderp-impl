...
#  if smoderp2d.constants.PARAMETER_ARCGIS == True: arcgis rasters are created \n
#  \n
...
def raster_output_arcgis(arrin, G, fs, outname, reachNA=True):
    ...
    saveAG = arcpy.NumPyArrayToRaster(
        tmparr, ll_corner, G.dx, G.dy, G.NoDataValue)
    saveAG.save(outName)


def raster_output_ascii(arrin, G, fs, outname, reachNA=True):
    output = G.outdir
    # KAvka - zm?nit na nazvy prom?nn?ch #jj pridal jsem jmena promennych do te tridy aspon muze byt vice lidsky ten nazev ....
    outName = output+os.sep+outname+".asc"
    wrk = arrin
    rrows = G.rr
    rcols = G.rc
    if (reachNA):
    ...
    for i in rrows:
        for j in rcols[i]:
            if cumulative.h_sur[i][j] == 0.:
                ...
                cumulative.v_sur[i][j] = cumulative.q_sur[i][j] / \
                    cumulative.h_sur[i][j]
            cumulative.shear_sur[i][j] = cumulative.h_sur[i][j] * \
                98.07 * mat_slope[i][j]

    #1, 2, 15, 16
    # jj vyznam najdes v class Cumulative mezi class Cumulative a def__init__
...
if stream and arcgis:
    ...
    def write_stream_table(outDir, surface, toky):
        ...
        arcpy.AddJoin_management(
            outTemp+"streamtmp.shp", "FID", outFile, "FID")
